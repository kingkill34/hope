...
public class UserDaoServicesImpl extends ResultObject implements UserDaoServices {
    ...
    @Override
    public ResultObject changePassword(Integer id, String oldPassword, String newPassword) {
        ResultObject resultObject = null;
        try {
            int result = userDao.changePassword(id, oldPassword, newPassword);
            if (result == 1) {
                resultObject = new ResultObject(UserPrompt.SUCCESS,
                                                UserPrompt.CHANGE_PASSWORD_SUCCESS);
            } else {
    ...
    public ResultObject addUser(String userName, String password) {
        ...
        // if (result == 1) {
        // resultObject = new
        // ResultObject(UserPrompt.SUCCESS,UserPrompt.ADD_USER_SUCCESS);
        // } else {
        // resultObject = new
        // ResultObject(UserPrompt.FAIL,UserPrompt.ADD_USER_ERROR);
        // }
        // } catch (Exception e) {
        // resultObject = new ResultObject(UserPrompt.FAIL, e);
        // }
